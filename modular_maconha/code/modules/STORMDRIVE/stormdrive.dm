...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/attackby(obj/item/I, mob/living/carbon/user, params)
	...
	if(I.tool_behaviour == TOOL_MULTITOOL)
		...
		M.buffer = src
		playsound(src, 'Reactor/flashlight_on.ogg', 100, TRUE)
	...
	if(istype(I, /obj/item/stack/sheet/metal))
		var/obj/item/stack/sheet/metal/D = I
		if(state == REACTOR_STATE_REPAIR)
			if(D.get_amount() < 25)
				to_chat(user, "<span class='notice'>You need at least twenty five pieces of metal to reline the reactor pit!</span>")
				return
			to_chat(user, "<span class='notice'>You start relining the reactor pit with metal...</span>")
			repairing = TRUE
			if(!do_after(user, 5 SECONDS, target=src) || !Adjacent(user))
				repairing = FALSE
				return
			D.use(25)
			to_chat(user, "<span class='notice'>The reactor pit has been relined with metal")
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/attack_hand(mob/living/carbon/user)
	...
	if(state >= REACTOR_STATE_MELTDOWN)
		return
	if(state >= REACTOR_STATE_RUNNING)
		to_chat(usr, "<span class='notice'>auch the [src] frame is hot!.</span>")
	ui_interact(user)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/try_start()
	...
	if(fuel_check >= start_threshold && heat >= start_threshold) //Checking equivalent of 20 moles of fuel and is hot enough
		...
		set_light(5)
		var/startup_sound = pick('Reactor/startup.ogg', 'Reactor/startup2.ogg')
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/lazy_startup() //Admin only command to instantly start a reactor
	...
	air1.adjust_moles(GAS_PLASMA, 500)
	try_start()
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/slowprocess()
	...
	if(state == REACTOR_STATE_MELTDOWN)
		radiation_pulse(src, (1000000 * radiation_modifier), 10)
		return
	...
	var/fuel_check = ((air1.get_moles(GAS_PLASMA) + air1.get_moles(GAS_PLUOXIUM) + air1.get_moles(GAS_TRITIUM)) / air1.total_moles()) * 100
	if(air1.total_moles() >= reaction_rate && fuel_check >= 12.5) //1:8 ratio
		...
		nucleium_power_reduction = reaction_chamber_gases.get_moles(GAS_TRITIUM) * 1000 //nucleium
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/can_cool()
	...
	if(control_rod_integrity < 0)
		...
		if(state == REACTOR_STATE_RUNNING)
			...
			send_alert("DANGER: insert a new control rod right now!")
		return FALSE
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/handle_ftl_fuel_production()
	if(heat > 30)
		...
		var/output_starting_pressure = air2.return_pressure()
		var/heat_kelvin = heat + 273.15
		var/fuel_amount = air1.get_moles(GAS_PLASMA) + air1.get_moles(GAS_PLUOXIUM) + air1.get_moles(GAS_TRITIUM)
		if(output_starting_pressure >= max_output_pressure) //if pressured capped, nucleium backs up into the drive
			air1.adjust_moles(GAS_NITRYL, ((fuel_amount / reaction_rate) / 10) * input_power_modifier)
			air1.set_temperature(heat_kelvin)
			update_parents()
		else
			air2.adjust_moles(GAS_NITRYL, (reaction_rate / 10) * input_power_modifier)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/handle_gas_records()
	if(world.time >= gas_records_next_interval)
		...
		var/list/constricted_plasma = gas_records["plasma"]
		constricted_plasma += (air1.get_moles(GAS_PLASMA) / air1.total_moles()) * 100
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/Bumped(atom/movable/A)
	visible_message("<span class='danger'>[A] starts to glow an ominous blue!</span>")
		...
		switch(reactor_stability)
			if(50 to INFINITY) //Concussions for the Concussiondrive
				if(C.get_bodypart(BODY_ZONE_HEAD))
					...
					visible_message("<span class='danger'>[A] head hit the outcasing of the [src]!</span>")
			...
			if(-INFINITY to 25) //Souls for the Souldrive
				...
				visible_message("<span class='danger'>[A] start to glow an ominous blue!</span>")
				C.gib()
				playsound(src, 'sound/effects/phasein.ogg', 100, TRUE) //temp - find a better sound
				...
				var/datum/gas_mixture/air1 = airs[1]
				air1.adjust_moles(GAS_PLASMA, 1000)
				return
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/examine(mob/user)
	. = ..()
	if(icon_state == "broken")
		. += "<span class='notice'>The reactor pit is full of plutonium sludge that sucks.</span>"
	switch(state)
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/start_meltdown()
	...
	//var/yourmom = 'Reactor/meltdown.ogg'
	SEND_SOUND(GLOB.player_list, 'Reactor/meltdown.ogg')
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/meltdown()
	if(heat >= reactor_temperature_meltdown)
		...
		update_icon()
		//var/KABBOM = 'Reactor/explode.ogg'
		SEND_SOUND(GLOB.player_list, 'Reactor/explode.ogg')
		cut_overlays()
		flick("meltdown", src)
		to_chat(GLOB.player_list, "<span class='boldannounce'>you hear a horrible metallic hissing you start to think someone screw up a stormdrive somewhere.</span>")
		do_meltdown_effects()
		sleep(30)
		icon_state = "broken"
...
/obj/machinery/atmospherics/components/binary/stormdrive_reactor/proc/do_meltdown_effects()
	var/kaboomsize = heat / 100
	explosion(get_turf(src), kaboomsize*1, kaboomsize*2, kaboomsize*3, kaboomsize*4, TRUE, TRUE)
	empulse(src, 60, 100)
	radiation_pulse(src, 10000, 1)
	src.atmos_spawn_air("o2=750;plasma=200;tritium=100;TEMP=1000000")
...
/obj/machinery/computer/ship/reactor_control_computer/attack_hand(mob/user)
	if(!allowed(user))
		var/sound = pick('Reactor/error.ogg','Reactor/error2.ogg','Reactor/error3.ogg')
...
/obj/machinery/computer/ship/reactor_control_computer/ui_data(mob/user)
	...
	data["fuel_mix"] = air1.get_moles(GAS_PLASMA) + air1.get_moles(GAS_PLUOXIUM) + air1.get_moles(GAS_TRITIUM)
...
/datum/weather/nuclear_fallout
	...
	weather_sound = 'Reactor/falloutwind.ogg'
	end_duration = 100
	area_type = /area
...
/obj/effect/anomaly/stormdrive/sheer/anomalyEffect()
	...
	if(prob(90))
		...
		atmos_spawn_air("tritium=15;TEMP=[temperature]")
...
/obj/effect/anomaly/stormdrive/sheer/detonate()
	...
	src.atmos_spawn_air("o2=125;plasma=50;tritium=50;TEMP=5000")
